...
if __name__ == "__main__":
    ...
    #   默认指向根目录下的VOC数据集
    #-------------------------------------------------------#
    VOCdevkit_path  = r'E:\Big_Datasets\water_surface\benchmark_new\WaterScenes_new'
    #-------------------------------------------------------#
    #   结果输出的文件夹，默认为map_out
    #-------------------------------------------------------#
    map_out_path    = 'map_out'

    image_ids = open(os.path.join(VOCdevkit_path, "test_list.txt")).read().strip().split()
    ...
    if map_mode == 0 or map_mode == 1:
        ...
        for image_id in tqdm(image_ids):
            image_path  = os.path.join(VOCdevkit_path, "images/"+image_id[9:25]+".jpg")
            image       = Image.open(image_path)
            if map_vis:
                image.save(os.path.join(map_out_path, "images-optional/" + image_id[9:25] + ".jpg"))
    ...
    if map_mode == 0 or map_mode == 2:
        ...
        for image_id in tqdm(image_ids):
            with open(os.path.join(map_out_path, "ground-truth/"+image_id[9:25]+".txt"), "w") as new_f:
                root = ET.parse(os.path.join(VOCdevkit_path, "detection/Annotations_0220/Annotations/"+image_id[9:25]+".xml")).getroot()
